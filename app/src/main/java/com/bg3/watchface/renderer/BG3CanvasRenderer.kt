...
class BG3CanvasRenderer(
    ...
    private fun renderHpGauge(canvas: Canvas, cx: Float, cy: Float, width: Float) {
        val radius = width * 0.415f
        arcBounds.set(cx - radius, cy - radius, cx + radius, cy + radius)
        val strokeW = width * 0.030f
        val startAngle = 138f
        val sweepAngle = 84f
        ...
        if (progressSweep > 0.5f) {
            paints.arcGlowPaint.strokeWidth = strokeW * 1.5f
        ...
        // Integrated HP Stat Badge - anchored directly to the inner arc with gothic clasp
        val badgeH = width * 0.125f
        val badgeRight = cx - radius * 0.56f
        val badgeLeft = cx - radius * 0.94f
        val badgeCx = (badgeLeft + badgeRight) / 2f
        val badgeRect = RectF(badgeLeft, cy - badgeH / 2f, badgeRight, cy + badgeH / 2f)

        // Clasp bracket physically connecting the arc bar to the stat card
        val arcInnerX = cx - radius + strokeW / 2f
        paints.d20InnerStrokePaint.color = currentTheme.hpRuby
        paints.d20InnerStrokePaint.strokeWidth = 2.5f
        canvas.drawLine(arcInnerX, cy, badgeLeft, cy, paints.d20InnerStrokePaint)
        canvas.drawCircle(badgeLeft, cy, 2.5f, paints.d20InnerStrokePaint.apply { color = currentTheme.goldPrimary })

        // Dark obsidian badge with ruby accent border
        paints.bannerBgPaint.color = Color.parseColor("#E60A0810")
        canvas.drawRoundRect(badgeRect, 8f, 8f, paints.bannerBgPaint)

        paints.d20InnerStrokePaint.color = (currentTheme.hpRuby and 0x00FFFFFF) or 0xCC000000.toInt()
        paints.d20InnerStrokePaint.strokeWidth = 1.8f
        canvas.drawRoundRect(badgeRect, 8f, 8f, paints.d20InnerStrokePaint)

        val hpTitle = if (batteryDisplayMode == BatteryDisplayMode.PERCENTAGE) "♥ HP" else "♥ BAT"
        val hpValue = if (batteryDisplayMode == BatteryDisplayMode.PERCENTAGE) {
        ...
        canvas.drawText(hpTitle, badgeCx, cy - badgeH * 0.13f, paints.subtextPaint)

        paints.badgePaint.textSize = width * 0.044f
        paints.badgePaint.color = currentTheme.goldLight
        canvas.drawText(hpValue, badgeCx, cy + badgeH * 0.32f, paints.badgePaint)
    }
    ...
    private fun renderXpGauge(canvas: Canvas, cx: Float, cy: Float, width: Float) {
        ...
        // Integrated XP Stat Badge - anchored directly to the inner arc with gothic clasp
        val badgeH = width * 0.125f
        val badgeLeft = cx + radius * 0.56f
        val badgeRight = cx + radius * 0.94f
        val badgeCx = (badgeLeft + badgeRight) / 2f
        val badgeRect = RectF(badgeLeft, cy - badgeH / 2f, badgeRight, cy + badgeH / 2f)

        // Clasp bracket physically connecting the arc bar to the stat card
        val arcInnerX = cx + radius - strokeW / 2f
        paints.d20InnerStrokePaint.color = currentTheme.xpArcane
        paints.d20InnerStrokePaint.strokeWidth = 2.5f
        canvas.drawLine(badgeRight, cy, arcInnerX, cy, paints.d20InnerStrokePaint)
        canvas.drawCircle(badgeRight, cy, 2.5f, paints.d20InnerStrokePaint.apply { color = currentTheme.goldPrimary })

        // Dark obsidian badge with arcane accent border
        paints.bannerBgPaint.color = Color.parseColor("#E60A0810")
        canvas.drawRoundRect(badgeRect, 8f, 8f, paints.bannerBgPaint)

        paints.d20InnerStrokePaint.color = (currentTheme.xpArcane and 0x00FFFFFF) or 0xCC000000.toInt()
        paints.d20InnerStrokePaint.strokeWidth = 1.8f
        canvas.drawRoundRect(badgeRect, 8f, 8f, paints.d20InnerStrokePaint)

        val xpTitle = when (stepsDisplayMode) {
            StepsDisplayMode.STEPS_XP -> "✦ XP"
            StepsDisplayMode.DISTANCE_KM -> "✦ KM"
            StepsDisplayMode.CALORIES_KCAL -> "✦ KCAL"
        }
        val xpValue = when (stepsDisplayMode) {
            StepsDisplayMode.STEPS_XP -> if (stepCount >= 1000) "${String.format(Locale.US, "%.1f", stepCount / 1000f)}k" else "$stepCount"
            StepsDisplayMode.DISTANCE_KM -> String.format(Locale.US, "%.1f", stepCount * 0.00075f)
            StepsDisplayMode.CALORIES_KCAL -> "${(stepCount * 0.045f).toInt()}"
        }

        paints.subtextPaint.textSize = width * 0.030f
        paints.subtextPaint.color = currentTheme.xpArcane
        canvas.drawText(xpTitle, badgeCx, cy - badgeH * 0.13f, paints.subtextPaint)

        paints.badgePaint.textSize = width * 0.044f
        paints.badgePaint.color = currentTheme.goldLight
        canvas.drawText(xpValue, badgeCx, cy + badgeH * 0.32f, paints.badgePaint)
    }
    ...
    private fun renderDigitalClock(canvas: Canvas, zdt: ZonedDateTime, cx: Float, height: Float, width: Float) {
        ...
        val timeY = height * 0.208f

        paints.timePaint.textSize = width * 0.142f
        paints.timeShadowPaint.textSize = width * 0.142f
        paints.timePaint.color = currentTheme.goldPrimary

        canvas.drawText(timeStr, cx + 2.5f, timeY + 3f, paints.timeShadowPaint)
        canvas.drawText(timeStr, cx, timeY, paints.timePaint)

        paints.subtextPaint.textSize = width * 0.033f
        ...
        canvas.drawText(badge, cx, timeY - paints.timePaint.textSize * 0.74f, paints.subtextPaint)
    }
    ...
    private fun renderSkillCheckBadge(canvas: Canvas, cx: Float, y: Float, width: Float) {
        ...
        paints.badgePaint.textSize = width * 0.038f
        paints.badgePaint.getTextBounds(badgeText, 0, badgeText.length, textBounds)
        val bw = textBounds.width() + 36f
        val bh = textBounds.height() + 16f

        val rect = RectF(cx - bw / 2f, y - bh / 2f, cx + bw / 2f, y + bh / 2f)
        paints.bannerBgPaint.color = Color.parseColor("#CC0A0A0E")
        canvas.drawRoundRect(rect, 7f, 7f, paints.bannerBgPaint)

        paints.d20InnerStrokePaint.color = currentTheme.goldDark
        paints.d20InnerStrokePaint.strokeWidth = 1.6f
        canvas.drawRoundRect(rect, 7f, 7f, paints.d20InnerStrokePaint)
    ...
    private fun renderBottomStatus(
        ...
        val weatherText = "${weather.condition.glyph} ${weather.displayTemp}"
        val metricsText = "$weatherText   •   ♥ 72 BPM"
        ...
        if (!isSettledOrCritical) {
            ...
            paints.subtextPaint.color = currentTheme.goldLight
            canvas.drawText(dateText, cx, dateY, paints.subtextPaint)

            val statusY = height * 0.875f
            paints.heartRatePaint.textSize = width * 0.036f
            paints.heartRatePaint.color = currentTheme.goldPrimary
            canvas.drawText(metricsText, cx, statusY, paints.heartRatePaint)
        } else {
            val statusY = height * 0.865f
            paints.heartRatePaint.textSize = width * 0.036f
            paints.heartRatePaint.color = currentTheme.goldPrimary
            canvas.drawText(metricsText, cx, statusY, paints.heartRatePaint)
        }
